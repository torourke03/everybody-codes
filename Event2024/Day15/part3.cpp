...
int main() {
    ...
    /*
     * The main hint for this one is that the input is 3 columns that only connect at the bottom row, 
     * so it should be possible to brute-force middle column herbs only, 
     * then try the same thing on the other columns, testing if left or right first is faster.
     * After a quick test, it should run *reasonably* quickly, since the smaller search only needs to run 6 times:
     * (Middle->Left, Left, Right), and (Middle->Right, Right, Left)
     */
    
    string line;
    for (int i = 0; getline(infile, line); i++) {
        line = line.substr(line.length() / 3, line.length() / 3);
